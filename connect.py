...
def pappers(name):
    search = requests.get(f"https://api.pappers.fr/v1/recherche?nom_entreprise={name}&api_token={config.api_pappers}")
    search = search.json()
    for i in range (len(search["entreprises"])):
        denom = search["entreprises"][0]["nom_entreprise"]
        siren = search["entreprises"][0]["siren"]
        code_naf = search["entreprises"][0]["code_naf"]
        print(denom,code_naf,siren)
